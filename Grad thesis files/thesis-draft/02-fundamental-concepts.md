...
The clinical content of the five AAMI beat classes is worth making concrete, because it explains why the rarity of some classes is unfortunate rather than incidental. The **N** class collects normal sinus beats together with bundle-branch-block beats, which are abundant. The **S** (supraventricular ectopic) and **V** (ventricular ectopic) classes are the diagnostically interesting middle ground: ventricular ectopy in particular can presage dangerous arrhythmias, yet it is far less common than normal conduction. The **F** (fusion) class — beats that are a hybrid of a normal and an ectopic activation — is genuinely rare, often well under one percent of beats, which is precisely the regime in which an accuracy-oriented classifier would simply learn to ignore it. The **Q** class gathers paced and otherwise unclassifiable beats. A clinically useful beat classifier must therefore perform well on the rare ectopic and fusion classes, not merely on the dominant normal class — a requirement that, as Section 2.2 makes explicit, the choice of evaluation metric must enforce.
...
The equal weighting is the crucial property: a rare but clinically important class contributes as much to macro F1 as the dominant normal class, so a model cannot score well by ignoring minorities. A complementary, threshold-independent measure is the **macro-averaged area under the receiver-operating-characteristic curve (AUC)**, computed one-versus-rest and averaged over classes; it summarizes ranking quality across all decision thresholds and is reported alongside macro F1 in this work as a secondary metric. Class imbalance is further mitigated during training by **inverse-frequency class weighting** of the cross-entropy loss, which up-weights the gradient contribution of under-represented classes so that a few abundant classes do not dominate the loss; the exact weighting used here is described in Chapter 5.

Two further practical levers shape how well such a network trains and generalizes, both used in this thesis. **Regularization** — weight decay, and dropout, which randomly zeros a fraction of activations during training — discourages over-fitting, which is a real risk on the rare classes where only a handful of examples exist. **Data augmentation** — small, label-preserving perturbations of the input signal — enlarges the effective training set and is used selectively here for the harder of the two record-level tasks. The interaction of these levers with the federated and differentially private settings is non-trivial, because both federation and DP noise already act as implicit regularizers, and Chapter 6 reports where additional capacity or augmentation did and did not help.
...
Under heterogeneity, the local optima $\theta_t^{(k)}$ that the clients reach in a round can point in conflicting directions; their average can be a poor compromise that fits no client well. This phenomenon, **client drift**, makes FedAvg converge more slowly and to a worse solution than centralized training, and the effect worsens as heterogeneity increases. The mechanism is easy to state: each client minimizes its *own* objective $F_k$, and when the $F_k$ are pulling toward different minima, several local SGD steps move each client toward its local minimum, so the average of the resulting parameters need not decrease the *global* objective $F$ at all. The more local computation per round — the larger $E$ — the more pronounced the drift, which is the central tension of communication-efficient FL: fewer rounds are cheaper but amplify the disagreement between clients.

This drift also exposes a subtlety that becomes the empirical centerpiece of this thesis. The global objective $F(\theta)$ measures performance on the *pooled* mixture of all clients' data, but in a heterogeneous deployment each client ultimately runs the model on its *own* distribution. A model that is poor on the pooled mixture may nonetheless be excellent on each client's individual distribution, and vice versa. Whether one cares about pooled performance or per-client performance is therefore not a detail of measurement but a choice of objective, and Chapter 6 shows that the choice can reverse the ranking of methods. Two influential responses to client drift frame the rest of this thesis.
...
The concrete threats that this guarantee defends against are not hypothetical. **Membership-inference attacks** can determine, from a trained model or its updates, whether a particular individual's record was in the training data [30] — already a serious disclosure when the dataset is, for example, a cohort of cardiac patients. **Gradient-inversion (or "deep leakage") attacks** go further and reconstruct individual training inputs from the gradients a client transmits [31], which is directly relevant to federated learning because gradients or parameter updates are exactly what clients share. Differential privacy is valuable precisely because its guarantee is *worst-case and attack-agnostic*: it bounds what any adversary, present or future, with any side information, can learn about any single record, rather than defending against a specific known attack. This is the property that makes it the standard for releasing models trained on sensitive medical data.
...
### 2.5.1 The granularity of privacy: record-level and client-level DP

A point that is easy to overlook, but that distinguishes this work from its closest prior art, is *what counts as one record* — that is, the **unit of adjacency** in the DP definition. Two granularities are standard in federated learning, and they protect different things.

Under **record-level (also example-level) DP**, two datasets are adjacent if they differ in a single training example — one heartbeat or one ten-second recording. A record-level guarantee bounds what an adversary can learn about any individual record. DP-SGD with **per-sample** gradient clipping, as developed above, provides exactly this: clipping the gradient of each example bounds the influence of that example.

Under **client-level (also party-level) DP**, two datasets are adjacent if they differ in one client's *entire* contribution — all of that institution's records at once. A client-level guarantee bounds what can be learned about whether a whole institution participated. The canonical mechanism, due to Geyer et al. [4] and McMahan et al. [32], clips and noises the *aggregated client update* rather than per-sample gradients: each client's whole update is treated as the sensitive object whose sensitivity is bounded.

These guarantees protect different units and are calibrated by different mechanisms, so they are not interchangeable; which one is appropriate depends on whether the privacy concern is the individual patient record or the participation of a whole site. The distinction is central to the positioning of this thesis. The "DP-FedBN" baseline that appears in the prior literature is a **client-level Laplace** mechanism on aggregated updates (Chapter 3), whereas the construction developed in Chapter 4 applies **record-level** DP-SGD via per-sample gradient clipping. Stating which granularity a method provides is therefore not pedantry; it is the difference between two genuinely distinct privacy claims.
